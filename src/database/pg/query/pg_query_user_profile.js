...
import { convertRowToUserProfileModel } from "../query_parser/user_profile_model.js";

export const getAUserProfileFilteredByIdQuery = async (id) => {
  let client;
  try {
    ...
    return result.rows.map(convertRowToUserProfileModel);
  } catch (error) {
...
export const getUserProfilesFilteredAutoQuery = async (
  name,
  email,
  offset = 0,
  limit
  ...
  try {
    ...
    return {
      places: result.rows.map(convertRowToUserProfileModel),
      meta: {
...
export const postAUserProfileQuery = async (name, email, image_id = null) => {
  let client;
  try {
    client = await pool.connect();
    const result = await client.query(
...
export const putAUserProfileQuery = async (name, email, image_id, id) => {
  let client;
  try {
    client = await pool.connect();
    const result = await client.query(
...
export const deleteAUserProfileQuery = async (id) => {
  let client;
  try {
    client = await pool.connect();
